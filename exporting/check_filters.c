...
int rrdset_is_exportable(struct instance *instance, RRDSET *st)
{
    ...
    if(unlikely(st->storage_engine_id == STORAGE_ENGINE_NONE && !(EXPORTING_OPTIONS_DATA_SOURCE(instance->config.options) == EXPORTING_SOURCE_DATA_AS_COLLECTED))) {
        debug(D_EXPORTING, "EXPORTING: not sending chart '%s' of host '%s' because its memory mode is '%s' and the exporting engine requires database access.", rrdset_id(st), rrdhost_hostname(host), storage_engine_name(host->storage_engine_id));
        return 0;
    }
