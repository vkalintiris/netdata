...
int setup_rrdhost()
{
    ...
    localhost->labels.label_list = label_list_new();
    label_list_add(localhost->labels.label_list, "key1", "value1", RRDLABEL_SOURCE_NETDATA_CONF);
    label_list_add(localhost->labels.label_list, "key2", "value2", RRDLABEL_SOURCE_AUTO);
...
int teardown_rrdhost()
{
    ...
    label_list_delete(localhost->labels.label_list);
