...
| C-6 | **Stale anchor cursor for an absent WAL tail → silent row drop, possibly optimistic `has_more`.** When a paginated `anchor` references a `Part::Tail` cursor whose WAL is no longer a query source (rotated into an SFST, or — post-C-5 — refused for that query), `materialize`'s `tail_by_seq.get(file_seq)` misses and the row is silently dropped by the final `filter_map`. `finalize_page`'s `has_more_in_direction` derives from the candidate set, so the opposite-side flag may be off-by-one, making a consumer "load more" return an empty page. **Pre-existing** — rotation is the dominant trigger; C-5's refuse-path only widened it slightly. Surfaced by the C-5 review (minimax). | `page.rs` (`materialize`, `finalize_page`) | Pre-existing; low priority. **Verify the off-by-one before fixing** — the silent drop is benign (one missing row for a stale anchor); the `has_more` claim is speculative. |
