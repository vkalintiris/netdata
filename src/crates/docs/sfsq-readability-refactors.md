...
| C-5 | **Multiple `WalTail`s sharing one `file_seq` collide in `materialize`.** The handler's `fallback` closure emits one `WalTail` per *failed* chunk build, all carrying `wal.seq` (`handler.rs:91-98`). Two such tails (a) collide in `tail_by_seq: HashMap<u64, &WalScan>` (last-write-wins, earlier scan's rows silently drop) and (b) emit cursors `(file_seq, Tail, position)` with per-scan insertion indices that overlap. **Pre-existing** — `git show 7ab2ee6a11:.../page.rs` has the identical `HashMap<u64>` keying; M-4 only renamed `seq`→`file_seq`. Surfaced by the M-4 review (minimax). Low frequency (needs ≥2 chunk-build failures for one WAL). Fix: route tails by `(file_seq, byte-range)` or a `Vec`+probe, and disambiguate tail-cursor positions. | `page.rs:278`, `handler.rs:91-98` | Pre-existing; out of scope for M-4. Separate ticket. |
