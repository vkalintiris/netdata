...
|----|------|----------|-------|
| M-1 | `paginate` is ~130 lines doing six jobs (partition, tail scan, SFST sort, open+evaluate+early-terminate, finalize, cold-release). Extract named helpers (`scan_tails`, `merge_sfsts`, `finalize`, `release_cold`) so the body reads as the documented map→reduce→root→fetch pipeline. The SFST loop resists extraction (its `IndexReader` borrows `Mapped` — both must share a scope). | `page.rs:275-414` | **Strong consensus across all review passes + internal analysis.** Pure structure, zero behavior change. |
| M-2 | `paginate` folds with `PageShard::merge(vec![merged, shard], …)` once per source, allocating a 2-element `Vec` each time. Add `PageShard::merge_into(&mut self, other, …)` and fold in place. **Verified faithful**: `PageShard` is exactly `{cursors, has_opposite}`; `merge` = extend + OR + `order_by_closeness` + truncate (`page.rs:55-70`), which `merge_into` replicates. | `page.rs:~303, ~341` | **Correction:** this is a *readability* win + a tiny alloc saving. It does **not** reduce sort count — `merge_into` still calls `order_by_closeness` per fold, exactly like today. Eliminating the per-fold re-sort is a *different* change (collect-all-then-sort-once) and is not this item. |
