...
| C-7 | **Log-table columns may serialize in the wrong order.** `otel-ledger` builds the wire `columns` object by inserting `timestamp`, `severity`, `cursor`, then fields in `index` order — but it does **not** enable `serde_json`'s `preserve_order` feature, so `serde_json::Map` is a `BTreeMap` and the keys serialize **alphabetically**, not in insertion/`index` order. The sibling `journal-function` crate enables `preserve_order` specifically for this, with a comment that the Netdata UI relies on column insertion order. Whether ours is actually wrong depends on whether the UI honors each column's `index` field (we set it) or the JSON key order. | `otel-ledger/Cargo.toml` (no `preserve_order`), `adapter.rs build_columns`; cf. `journal-function/.../columns.rs` | **Next up.** Verify the UI's column-ordering behavior, then enable `preserve_order` if needed. Surfaced by the M-8 consult. |
