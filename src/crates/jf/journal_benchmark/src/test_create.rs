...
fn main() -> error::Result<()> {
    ...
    test_data.insert("MESSAGE", vec!["Hello, world!", "Another message", "Final message"]);
    test_data.insert("PRIORITY", vec!["6", "4", "3"]);
    test_data.insert("_SYSTEMD_UNIT", vec!["test.service", "other.service", "test.service"]);
    ...
    println!("Created test journal file '{}' with {} entries", output_path, iterations * num_entries as u64);
    println!("Now run: cargo run --bin journal_benchmark -- {}", output_path);
