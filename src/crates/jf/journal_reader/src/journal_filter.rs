...
impl FilterExpr {
    ...
    pub fn next<M: MemoryMap>(&mut self, object_file: &ObjectFile<M>) -> Result<Option<u64>> {
        match self {
    ...
    pub fn prev<M: MemoryMap>(&mut self, object_file: &ObjectFile<M>) -> Result<Option<u64>> {
        match self {
            FilterExpr::Match(data_offset, cursor_opt) => {
