...
async fn run_plugin() -> std::result::Result<(), Box<dyn std::error::Error>> {
    ...
    info!(
        "configuration loaded: journal_paths={:?}, memory_capacity={}, workers={}, max_unique_values_per_field={}, max_field_payload_size={}",
    ...
    // Create catalog function with in-memory LRU cache
