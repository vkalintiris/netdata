...
fn blacklisted(guid: &str) -> bool {
    let found = BLACKLISTED.contains(&guid);
    if found {
        nd_log!(
            Source::Daemon,
            Priority::Info,
            "MACHINE_GUID: blacklisted machine GUID '{guid}' found, generating new one."
        );
    }
    found
}
...
fn write_to_file(dir: &Path, filename: &Path, guid: &str, modified_ut: u64) -> bool {
    ...
    if file.write_all(guid.as_bytes()).is_err() {
        nd_log!(
            Source::Daemon,
            Priority::Err,
            "MACHINE_GUID: cannot write GUID to the temporary GUID file '{tmp}'"
        );
        return unlink("write failure");
    }
        ...
        .is_err()
    {
        nd_log!(
            Source::Daemon,
            Priority::Err,
            "MACHINE_GUID: cannot set permissions on temporary GUID file '{tmp}'"
        );
        return unlink("permission failure");
    }
    ...
    if file.sync_all().is_err() {
        nd_log!(
            Source::Daemon,
            Priority::Err,
            "MACHINE_GUID: cannot synchronize temporary GUID file '{tmp}'"
        );
        return unlink("synchronization failure");
    }
    drop(file);
    if fs::rename(&tmp, filename).is_err() {
        ...
        return unlink("publication failure");
    }
    match File::open(dir) {
    ...
    nd_log!(
        Source::Daemon,
        Priority::Info,
        "MACHINE_GUID: GUID saved to file '{}'",
        filename.display()
    );
    true
}
...
pub fn machine_guid_get(varlib: &str) -> String {
    ...
    if let Err(e) = fs::DirBuilder::new().mode(0o775).create(&dir) {
        if e.kind() != io::ErrorKind::AlreadyExists {
            nd_log!(
                Source::Daemon,
                Priority::Err,
                "MACHINE_GUID: cannot create directory '{}'",
                dir.display()
            );
            return guid;
        }
    ...
    let Some(_lock) = lock else {
        nd_log!(
            Source::Daemon,
            Priority::Err,
            "MACHINE_GUID: cannot acquire publication lock '{lock_filename}'"
        );
        return guid;
    };
