...
static inline std::string anyValueToString(const pb::AnyValue &V)
{
    ...
    fatal("Unimplemented conversion of any value to string...");
}
...
public:
    ...
    inline absl::StatusOr<std::string> instanceName(const std::set<std::string> *Keys) const
    {
        ...
        for (const auto &Key : *Keys) {
            ...
            if (Underscore)
                Name += "_";
            Name += anyValueToString(*Result.value());
            Underscore = true;
        }
...
struct OtelElement {
    ...
    const std::string *DimsAttr;
    const std::set<std::string> *InstAttrs;

    OtelElement() : MD(nullptr), RM(nullptr), SM(nullptr), M(nullptr), DP(), DimsAttr(nullptr), InstAttrs(nullptr)
    {
    }
...
public:
    ...
    absl::StatusOr<std::string> dimensionName() const
    {
        if (!DimsAttr)
            return DataPoint::DefaultDimensionName;

        return DP.dimensionName(DimsAttr);
    }

    absl::StatusOr<std::string> instanceName() const
    {
        std::string Name = SM->scope().name() + "_" + M->name();

        if (!InstAttrs) {
            return Name;
        }

        const auto &Result = DP.instanceName(InstAttrs);
        if (!Result.ok())
            return Result.status();
...
class OtelData {
    ...
    public:
        explicit Iterator(const otel::config::Config *Cfg, MetricsDataIterator MDBegin, MetricsDataIterator MDEnd)
            : Cfg(Cfg), MDIt(MDBegin), MDEnd(MDEnd), DPKind(DataPointKind::NotAvailable), End(true)
        {
            if (MDIt != MDEnd) {
    ...
    private:
        ...
        OtelElement next()
        {
            ...
            if (!CurrOE.M || (CurrOE.M != OE.M)) {
                OE.DimsAttr = nullptr;
                OE.InstAttrs = nullptr;

                if (OE.SM->has_scope()) {
                    const otel::config::Metric *CfgMetric = Cfg->getMetric(OE.SM->scope().name(), OE.M->name());
                    if (CfgMetric) {
                        OE.DimsAttr = CfgMetric->getDimensionsAttribute();
                        OE.InstAttrs = CfgMetric->getInstanceAttributes();
                    }
                }
            } else {
                OE.DimsAttr = CurrOE.DimsAttr;
                OE.InstAttrs = CurrOE.InstAttrs;
            }
        ...
        void advanceIterators()
        {
            switch (DPKind) {
                ...
                case DataPointKind::Exponential:
                    ...
                    break;
                case DataPointKind::NotAvailable:
                    netdata_log_error("advanceIterators(): not available");
                    break;
            }
...
public:
    OtelData(const otel::config::Config *Cfg, const std::vector<pb::MetricsData> &MDV) : Cfg(Cfg), MDV(MDV)
    {
    }

    inline Iterator begin()
    {
        return Iterator(Cfg, MDV.begin(), MDV.end());
    }

    inline Iterator end() const
    {
        return Iterator(Cfg, MDV.end(), MDV.end());
    }

private:
    const otel::config::Config *Cfg;
    const std::vector<pb::MetricsData> &MDV;
};
