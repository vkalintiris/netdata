...
static void metadata_event_loop(void *arg)
{
    ...
    worker_register_job_name(METADATA_ML_LOAD_MODELS,       "load ml models");
    worker_register_job_name(METADATA_ML_UPDATE_MODELS,     "update ml models");
    ...
    while (shutdown == 0 || (wc->flags & METADATA_WORKER_BUSY)) {
        ...
        do {
            ...
            switch (opcode) {
                ...
                case METADATA_ML_UPDATE_MODELS: {
                    RRDDIM *rd = (RRDDIM *) cmd.param[0];
                    ml_dimension_update_models(rd);
...
void metaqueue_ml_update_models(RRDDIM *rd)
{
    if (unlikely(!metasync_worker.loop))
        return;

    queue_metadata_cmd(METADATA_ML_UPDATE_MODELS, rd, NULL);
}
