...
#define GORILLA_PAGE_SIZE_IN_BYTES    512
#define GORILLA_PAGE_SIZE_IN_WORDS    128
...
void pgd_init_arals(void)
{
    ...
    // gorilla aral
    {
        ...
        // FIXME: add stats
        size_t gorilla_page_size = GORILLA_PAGE_SIZE_IN_BYTES;
...
PGD *pgd_create(uint8_t type, uint32_t slots)
{
    ...
    switch (type) {
        ...
        case PAGE_GORILLA_METRICS: {
            ...
            memset(gbuf, 0, GORILLA_PAGE_SIZE_IN_BYTES);

            *pg->gorilla.writer = gorilla_writer_init(gbuf, GORILLA_PAGE_SIZE_IN_WORDS);
    ...
    switch (pg->type) {
        ...
        case PAGE_GORILLA_METRICS: {
            if (pg->states & PGD_STATE_CREATED_FROM_DISK)
                ...
                footprint = sizeof(PGD) + sizeof(gorilla_writer_t) + (pg->gorilla.num_buffers * GORILLA_PAGE_SIZE_IN_BYTES);
    ...
    switch (pg->type) {
        ...
        case PAGE_GORILLA_METRICS: {
                ...
                pg->states & PGD_STATE_FLUSHED_TO_DISK)
            {
                ...
                size = pg->gorilla.num_buffers * GORILLA_PAGE_SIZE_IN_BYTES;
    ...
    switch (pg->type) {
        ...
        case PAGE_GORILLA_METRICS: {
            ...
            if (!ok) {
                ...
                memset(new_buffer, 0, GORILLA_PAGE_SIZE_IN_BYTES);

                gorilla_writer_add_buffer(pg->gorilla.writer, new_buffer, GORILLA_PAGE_SIZE_IN_WORDS);
                pg->gorilla.num_buffers += 1;
