...
static const char *rdb_path = "/home/vk/repos/nd/tmp/rdb";

static DB *rdb = nullptr;

static ColumnFamilyHandle *mcf = nullptr;
static ColumnFamilyHandle *rcf = nullptr;
...
rdb_instance_t *rdb_init()
{
    ...
    column_families.push_back(ColumnFamilyDescriptor(ROCKSDB_NAMESPACE::kDefaultColumnFamilyName, ColumnFamilyOptions()));
    column_families.push_back(ColumnFamilyDescriptor("rcf", ColumnFamilyOptions()));
    column_families.push_back(ColumnFamilyDescriptor("mcf", ColumnFamilyOptions()));

    std::vector<ColumnFamilyHandle *> handles;
    Status s = DB::Open(options, rdb_path, column_families, &handles, &rdb);
    ...
    if (s.ok()) {
        ...
        s = DB::Open(options, rdb_path, &rdb);
        if (!s.ok())
            return nullptr;

        s = rdb->CreateColumnFamily(ColumnFamilyOptions(), "mcf", &mcf);
        assert(s.ok());

        s = rdb->CreateColumnFamily(ColumnFamilyOptions(), "rcf", &rcf);
        assert(s.ok());
    }

    return reinterpret_cast<rdb_instance_t *>(rdb);
}
...
void rdb_fini()
{
    ...
    s = rdb->DropColumnFamily(mcf);
    assert(s.ok());

    s = rdb->DropColumnFamily(rcf);

    delete rdb;
}

struct rdb_metric_handle_t {
    uuid_t uuid;
};

STORAGE_METRIC_HANDLE *
rdb_metric_get(STORAGE_INSTANCE *db_instance __maybe_unused, uuid_t *uuid)
{
    DB *db = reinterpret_cast<DB *>(db_instance);

    std::string v;
    Status s = rdb->Get(ReadOptions(), mcf, (const char * ) uuid, &v);

    if (!s.IsNotFound())
        return nullptr;

    rdb_metric_handle_t *metric = static_cast<rdb_metric_handle_t *>(callocz(1, sizeof(rdb_metric_handle_t)));
    uuid_parse(v.data(), metric->uuid);

    return reinterpret_cast<STORAGE_METRIC_HANDLE *>(metric);
}

STORAGE_METRIC_HANDLE *
rdb_metric_get_or_create(RRDDIM *rd, STORAGE_INSTANCE *db_instance __maybe_unused)
{
    (void) rd;
    (void) db_instance;

    STORAGE_METRIC_HANDLE *smh = rdb_metric_get(db_instance, &rd->metric_uuid);

    if (!smh) {
        rdb_metric_handle_t *rmh = static_cast<rdb_metric_handle_t *>(callocz(1, sizeof(rdb_metric_handle_t)));
        internal_fatal(rmh == NULL, "OOM");
        uuid_copy(rmh->uuid, rd->metric_uuid);

        Status s = rdb->Put(WriteOptions(), mcf, (const char *) &rd->metric_uuid, "");
        assert(s.ok());

        smh = reinterpret_cast<STORAGE_METRIC_HANDLE *>(rmh);
    }

    return smh;
}

void rdb_metric_release(STORAGE_METRIC_HANDLE *db_metric_handle __maybe_unused)
{
    rdb_metric_handle_t *rmh = reinterpret_cast<rdb_metric_handle_t *>(db_metric_handle);
    freez(rmh);
}
...
bool rdb_metric_retention_by_uuid(STORAGE_INSTANCE *db_instance __maybe_unused, uuid_t *uuid, time_t *first_entry_s, time_t *last_entry_s)
{
    (void) db_instance;
    (void) uuid;
    (void) first_entry_s;
