...
// TODO: 1 - we only have one prediction thread now.
//       2 - how are we going to handle training threads?
void nml_update_resource_usage_charts(RRDHOST *host, const struct rusage &PredictionRU, const struct rusage &TrainingRU) {
#if 1
    UNUSED(host);
    UNUSED(PredictionRU);
    UNUSED(TrainingRU);
