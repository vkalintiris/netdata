...
static bool
should_predict_set(RRDSET *st, size_t train_every) {
    ...
    if (num_dims == 0) {
        info("will not predict set \"%s\" because it has zero dims", name);
        return false;
    }
...
static bool
predict_chart(RRDSET *st, size_t num_samples, size_t train_every,
              size_t diff_n, size_t smooth_n, size_t lag_n) {
    return false;

#if 0
    if (!should_predict_set(st, train_every))
        return false;

    RRDR *res = get_rrdr(st, diff_n + smooth_n + lag_n);
    if (!res)
        return false;
...
static void
predict_charts(size_t num_samples, size_t train_every,
               size_t diff_n, size_t smooth_n, size_t lag_n) {
    size_t num_predicted_charts = 0;

    rrdhost_rdlock(localhost);

    RRDSET *st;
    rrdset_foreach_read(st, localhost) {
        if (predict_chart(st, num_samples, train_every, diff_n, smooth_n, lag_n))
            num_predicted_charts++;
    }

    rrdhost_unlock(localhost);

    info("Predicted %zu charts", num_predicted_charts);
}
...
ml_predict(void *ptr) {
    ...
    num_samples = config_get_number(CONFIG_SECTION_ML, "num samples to train", 300);
    train_every = config_get_number(CONFIG_SECTION_ML, "train every secs", 30);
    ...
    lag_n = config_get_number(CONFIG_SECTION_ML, "num samples to lag", 5);

    while (train_heartbeat())
        predict_charts(num_samples, train_every, diff_n, smooth_n, lag_n);
