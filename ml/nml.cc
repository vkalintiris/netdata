...
void nml_host_start_anomaly_detection_threads(nml_host_t *host) {
    ...
    netdata_thread_create(&host->detection_thread, tag, NETDATA_THREAD_OPTION_JOINABLE, detect_main, static_cast<void *>(host));
}
...
void nml_host_stop_anomaly_detection_threads(nml_host_t *host, bool join) {
    ...
    if (join && !host->threads_joined) {
        ...
        netdata_thread_join(host->detection_thread, NULL);
    }
}
