...
static const char *mls2str(enum nml_machine_learning_status mls) {
    switch (mls) {
...
static const char *mt2str(enum nml_metric_type mt) {
    switch (mt) {
...
static const char *ts2str(enum nml_training_status ts) {
    switch (ts) {
...
static const char *tr2str(enum nml_training_result tr) {
    switch (tr) {
...
static void nml_features_lag(nml_features_t *features) {
    ...
    features->preprocessed_features = std::vector<DSample>(n);
...
nml_training_request_t nml_queue_pop(nml_queue_t *q) {
    ...
    while (q->internal.empty()) {
        ...
        if (__atomic_load_n(&q->exit, __ATOMIC_SEQ_CST)) {
            netdata_mutex_unlock(&q->mutex);
            pthread_exit(NULL);
        }
    }

    nml_training_request_t req = q->internal.front();
    q->internal.pop();
...
void nml_queue_signal(nml_queue_t *q) {
    netdata_mutex_lock(&q->mutex);
    __atomic_store_n(&q->exit, true, __ATOMIC_SEQ_CST);
    pthread_cond_signal(&q->cond_var);
    netdata_mutex_unlock(&q->mutex);
}
...
nml_dimension_calculated_numbers(nml_dimension_t *dim, const nml_training_request_t &training_request) {
    ...
    */
    ...
    size_t idx = 0;
    calculated_number_t *cns = new calculated_number_t[max_n * (Cfg.lag_n + 1)]();
    ...
    for (idx = 0; std::isnan(cns[idx]); idx++, training_response.total_values--) { }

    // Overwrite NaN values.
    if (idx != 0)
        memmove(cns, &cns[idx], sizeof(calculated_number_t) * training_response.total_values);

    training_response.result = TRAINING_RESULT_OK;
    return { cns, training_response };
}
...
nml_dimension_train_model(nml_dimension_t *dim, const nml_training_request_t &training_request) {
    ...
    calculated_number_t *cns = P.first;
    ...
    unsigned N = training_response.total_values;
#if 0
     ...
     */
    {
        calculated_number_t *Src = new calculated_number_t[128 * 1024];
        calculated_number_t *Dst = new calculated_number_t[128 * 1024];

        memcpy(Src, cns, N * sizeof(calculated_number_t));
        memcpy(Dst, cns, N * sizeof(calculated_number_t));

        std::vector<DSample> DSV;
...
bool nml_dimension_predict(nml_dimension_t *dim, time_t curr_time, calculated_number_t value, bool exists) {
    ...
    // Create the sample
    calculated_number_t src_cns[128 * 1024];
    calculated_number_t dst_cns[128 * 1024];
...
void nml_dimension_dump(nml_dimension_t *dim) {
    ...
    const char *mls_str = mls2str(dim->mls);
    const char *mt_str = mt2str(dim->mt);
    const char *ts_str = ts2str(dim->ts);
    const char *tr_str = tr2str(dim->tr.result);
...
static void nml_host_detect_once(nml_host_t *host) {
    ...
    host->mls = {};
    nml_machine_learning_stats_t MLSCopy = {};
    nml_training_stats_t TSCopy = {};
    ...
    // Calc the avg values
    if (TSCopy.num_popped_items) {
        TSCopy.queue_size /= TSCopy.num_popped_items;
        TSCopy.allotted_ut /= TSCopy.num_popped_items;
        TSCopy.consumed_ut /= TSCopy.num_popped_items;
        TSCopy.remaining_ut /= TSCopy.num_popped_items;

        TSCopy.training_result_ok /= TSCopy.num_popped_items;
        TSCopy.training_result_invalid_query_time_range /= TSCopy.num_popped_items;
        TSCopy.training_result_not_enough_collected_values /= TSCopy.num_popped_items;
        TSCopy.training_result_null_acquired_dimension /= TSCopy.num_popped_items;
        TSCopy.training_result_chart_under_replication /= TSCopy.num_popped_items;
    } else {
        TSCopy.queue_size = 0;
        TSCopy.allotted_ut = 0;
        TSCopy.consumed_ut = 0;
        TSCopy.remaining_ut = 0;
    }
    ...
    nml_update_dimensions_chart(host->rh, MLSCopy);

    worker_is_busy(WORKER_JOB_DETECTION_HOST_CHART);
    nml_update_host_and_detection_rate_charts(host->rh, host->host_anomaly_rate * 10000.0);
    ...
    nml_update_training_statistics_chart(host->rh, TSCopy);
}
...
void nml_host_train(nml_host_t *host) {
    ...
    while (service_running(SERVICE_ML_TRAINING)) {
        nml_training_request_t TrainingReq = nml_queue_pop(&host->training_queue);
        size_t Size = nml_queue_size(&host->training_queue) + 1;

        usec_t AllottedUT = (Cfg.train_every * host->rh->rrd_update_every * USEC_PER_SEC) / Size;
        if (AllottedUT > USEC_PER_SEC)
            AllottedUT = USEC_PER_SEC;

        usec_t StartUT = now_monotonic_usec();
        enum nml_training_result TrainingRes;
        {
            worker_is_busy(WORKER_JOB_TRAINING_FIND);
            nml_acquired_dimension_t acq_dim = nml_acquired_dimension_get(host->rh, TrainingReq.chart_id, TrainingReq.dimension_id);

            worker_is_busy(WORKER_JOB_TRAINING_TRAIN);
            TrainingRes = nml_acquired_dimension_train(acq_dim, TrainingReq);

            string_freez(TrainingReq.chart_id);
            string_freez(TrainingReq.dimension_id);
...

        {
            ...
            host->ts.allotted_ut += AllottedUT;
            host->ts.consumed_ut += ConsumedUT;
            host->ts.remaining_ut += RemainingUT;
...
static void nml_host_detect(nml_host_t *host) {
    ...
    while (service_running((SERVICE_TYPE)(SERVICE_ML_PREDICTION | SERVICE_COLLECTORS))) {
        worker_is_idle();
        heartbeat_next(&HB, host->rh->rrd_update_every * USEC_PER_SEC);
        nml_host_detect_once(host);
    }
}
