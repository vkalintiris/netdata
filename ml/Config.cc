...
void Config::readMLConfig(void) {
    ...
    bool EnableDBEngine = config_get_boolean(ConfigSectionML, "dbengine enabled", true);
    ...
    unsigned DBEngineAnomalyRateEvery = config_get_number(ConfigSectionML, "dbengine anomaly rate every", 2);
