...
// TODO: replace this with NETDATA_INTERNAL_CHECKS
#if 1
...
/*
 * TODO:
 *  - match parser on parent
 *  - test/fix bugs
 *  - handle timestamps
 *  - use buffer_rrd_value to format long doubles
 *  - send storage_number flags (do we really need to?)
 */
void replicate_chart_response(RRDHOST *host, RRDSET *st,
                              bool start_streaming, time_t after, time_t before)
{
    ...
    // we might want to optimized this by filling a temporary buffer
...

    {
        ...
        fprintf(stderr, "\n------------------------------------\n");
        fprintf(stderr, "%s", buffer_tostring(tmp_wb));
        fprintf(stderr, "------------------------------------\n");
        fflush(stderr);
    }
