                                           ...
                                           int current_region, time_t time_start)
{
    ...
    for (i = 0 ; i < CHARTS ; ++i) {
        for (j = 0 ; j < DIMS ; ++j) {
            storage_engine_store_change_collection_frequency(rd[i][j]->tiers[0].db_metric_handle,
                                       ...
                                       int current_region, time_t time_start)
{
    ...
    for (c = 0; c < REGION_POINTS[current_region] ; c += QUERY_BATCH) {
        ...
        for (i = 0 ; i < CHARTS ; ++i) {
            for (j = 0; j < DIMS; ++j) {
                storage_engine_query_init(rd[i][j]->tiers[0].backend,
                                          rd[i][j]->tiers[0].db_metric_handle,
                                          rd[i][j]->tiers[0].db_collection_handle,
                                          &handle,
                                          time_now,
                                          time_now + QUERY_BATCH * update_every,
