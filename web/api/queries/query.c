...
static void query_planer_initialize_plans(QUERY_ENGINE_OPS *ops) {
    ...
    for(size_t p = 0; p < qm->plan.used ; p++) {
        ...
        storage_engine_query_init(eng->backend,
...
void rrdr_fill_tier_gap_from_smaller_tiers(RRDDIM *rd, size_t tier, time_t now_s) {
    ...
    time_t latest_time_s = storage_engine_latest_time_s(t->backend, t->db_metric_handle, t->db_collection_handle);
    ...
    for(int read_tier = (int)tier - 1; read_tier >= 0 ; read_tier--){
        ...
        time_t smaller_tier_last_time = storage_engine_latest_time_s(rd->tiers[read_tier].backend,
                                                                     rd->tiers[read_tier].db_metric_handle,
                                                                     rd->tiers[read_tier].db_collection_handle);
        ...
        storage_engine_query_init(tmp->backend,
                                  tmp->db_metric_handle,
                                  tmp->db_collection_handle,
                                  &handle,
                                  after_wanted,
                                  before_wanted,
