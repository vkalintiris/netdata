...
def _static_build_bash(client: dagger.Client, ctr: dagger.Container) -> dagger.Container:
    tree = (
...
def _static_build_curl(client: dagger.Client, ctr: dagger.Container) -> dagger.Container:
    tree = (
...
def _static_build_libnetfilter_acct(
    client: dagger.Client, ctr: dagger.Container
) -> dagger.Container:
    tree = (
...
def _static_build(client, repo_path):
    cmake_build_release_path = os.path.join(repo_path, "cmake-build-release")

    ctr = alpine_3_18(client, dagger.Platform("linux/x86_64"))
    ctr = _static_build_openssl(client, ctr)
    ctr = _static_build_bash(client, ctr)
    ctr = _static_build_curl(client, ctr)
    ctr = _static_build_ioping(client, ctr)
    ctr = _static_build_libnetfilter_acct(client, ctr)
    ...
    ctr = _static_build_netdata(client, ctr)
...
def amazon_linux_2(
    ...
    checksum_path = resources.resource_path(f"distro/cmake-{machine}.sha256")

    ctr = (
        ctr.with_file(
            f"cmake-{machine}.sha256",
            client.host().file(checksum_path),
        )
        .with_exec(
            [
                "curl",
...
def centos_7(
    ...
    centos_7_repo_path = resources.resource_path("distro/centos-7.repo")
    centos_7_scl_repo_path = resources.resource_path("distro/centos-7-scl.repo")
    signature_path = resources.resource_path("distro/RPM-GPG-KEY-CentOS-SIG-SCLo")
    ...
    ctr = (
        ...
        .with_file(
            ...
            client.host().file(centos_7_repo_path),
        )
        .with_file(
            "/etc/yum.repos.d/centos-7-scl.repo",
            client.host().file(centos_7_scl_repo_path),
        )
        .with_file(
            "/etc/pki/rpm-gpg/RPM-GPG-KEY-CentOS-SIG-SCLo",
            client.host().file(signature_path),
        )
        .with_exec(["yum", "install", "-y", "epel-release"])
...
def oracle_linux_9(
    ...
    repo_path = resources.resource_path("distro/ol9-epel.repo")

    ctr = (
        ctr.with_file(
            "/etc/yum.repos.d/ol9-epel.repo",
            client.host().file(repo_path),
        )
...
def oracle_linux_8(
    ...
    repo_path = resources.resource_path("distro/ol8-epel.repo")
...
def opensuse_15_6(
    ...
    ctr = client.container(platform=platform).from_("opensuse/leap:15.6")
...
def ubuntu_24_04(
    ...
    ctr = client.container(platform=platform).from_("ubuntu:24.04")
...
def ubuntu_24_10(
    ...
    ctr = client.container(platform=platform).from_("ubuntu:24.10")
