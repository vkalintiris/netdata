...
from . import envs
from .distros import SPECS, Distro, DistroSpec
...
def configure_args(spec: DistroSpec, platform: str, build_type: str = "Debug") -> list[str]:
    features: dict[str, bool] = {name: True for name in _COMMON_ON}
    features["PLUGIN_SYSTEMD_JOURNAL"] = spec.systemd
    features["PLUGIN_SYSTEMD_UNITS"] = spec.systemd
    ...
    args += envs.compiler_launcher_args(envs.has_ccache(spec.build))
...
def source_build(
    ...
    key = f"{distro.value}-{platform.replace('/', '-')}"
    ctr = (
        envs.with_build_caches(envs.bootstrap(spec.build, platform), key)
        .with_directory(SRC_DIR, source)
        .with_workdir(SRC_DIR)
        .with_env_variable("DISABLE_TELEMETRY", "1")
        .with_exec(configure_args(spec, platform, build_type))
