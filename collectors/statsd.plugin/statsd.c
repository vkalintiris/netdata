...
void *statsd_main(void *ptr) {
    ...
    statsd.update_every = rrdb.default_rrd_update_every;
    statsd.update_every = (int)config_get_number(CONFIG_SECTION_STATSD, "update every (flushInterval)", statsd.update_every);
    if(statsd.update_every < rrdb.default_rrd_update_every) {
        collector_error("STATSD: minimum flush interval %d given, but the minimum is the update every of netdata. Using %d", statsd.update_every, rrdb.default_rrd_update_every);
        statsd.update_every = rrdb.default_rrd_update_every;
    }
