...
public:
    ...
    void update(struct timeval WallTime, size_t Iteration) {
        UNUSED(Iteration);
        collected_number CN = ++NumUpdates; // % 5 ? Index + 1 : Index;
        rrddim_timed_set_by_pointer(RD->rrdset, RD, WallTime, CN);
...
public:
    ...
    void update(struct timeval WallTime, size_t Iteration) {
        ...
        if (Initialized) {
            ...
            rrdset_timed_next(RS, WallTime, DurationSinceLastUpdate);
        }

        Initialized = true;

        for (std::shared_ptr<Dimension> &D : Dimensions)
            D->update(WallTime, Iteration);

        rrdset_timed_done(RS, WallTime);
    }
...
static void updateCharts(std::vector<std::shared_ptr<Chart>> &Charts,
                         struct timeval WallTime, size_t Iteration)
{
    for (std::shared_ptr<Chart> &C : Charts)
        C->update(WallTime, Iteration);
}
...
void *profile_main(void *ptr)
{
    ...
    now_realtime_timeval(&WallTime);
    WallTime.tv_sec -= WallTime.tv_sec % UpdateEvery;
    WallTime.tv_sec -= 30 * 24 * 3600 * UpdateEvery;
    WallTime.tv_usec = 0;
    ...
    while (!netdata_exit) {
        ...
        now_realtime_timeval(&CurrTime);
        CurrTime.tv_usec = 0;

        if (WallTime.tv_sec >= CurrTime.tv_sec) {
            error("Live data @%ld", WallTime.tv_sec);
            heartbeat_next(&HB, UpdateEvery * USEC_PER_SEC);
        } else {
            time_t Rem = CurrTime.tv_sec - WallTime.tv_sec;
            if ((Rem % 3600) == 0)
                error("Remaining time: %ld hours", Rem / 3600);
        }

        updateCharts(Charts, WallTime, ++Iteration);
        WallTime.tv_sec += UpdateEvery;
    }
