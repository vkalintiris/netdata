...
#define GBUF_ELEMS 128
#define GBUF_BYTES (GBUF_ELEMS * sizeof(uint32_t))
...
uint32_t gorilla_buffer_patch(gorilla_buffer_t *gbuf) {
    ...
    while (curr_gbuf->header.next) {
        ...
        gbuf = reinterpret_cast<gorilla_buffer_t *>(&buf[GBUF_ELEMS]);
