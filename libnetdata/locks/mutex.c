        ...
        else
        {
            if(old != PTHREAD_CANCEL_DISABLE)
            {
                ...
                netdata_log_error("THREAD_CANCELABILITY: netdata_thread_enable_cancelability(): old thread cancelability "
                                  ...
                                  netdata_thread_tag(), PTHREAD_CANCEL_DISABLE, (old == PTHREAD_CANCEL_ENABLE) ? "ENABLED" : "UNKNOWN", old);
            }
        }
    }
...
static netdata_rwlock_locker *find_rwlock_locker(const char *file __maybe_unused, const char *function __maybe_unused,
...
static netdata_rwlock_locker *add_rwlock_locker(const char *file, const char *function,
                                                const unsigned long line, netdata_rwlock_t *rwlock, LOCKER_REQUEST lock_type)
{
    ...
    else
    {
        ...
        *PValue = locker;
        ...
        if (lock_type == RWLOCK_REQUEST_READ || lock_type == RWLOCK_REQUEST_TRYREAD)
        {
            rwlock->readers++;
        }

        if (lock_type == RWLOCK_REQUEST_WRITE || lock_type == RWLOCK_REQUEST_TRYWRITE)
        {
            rwlock->writers++;
        }
...
static void remove_rwlock_locker(const char *file __maybe_unused, const char *function __maybe_unused,
                                 const unsigned long line __maybe_unused, netdata_rwlock_t *rwlock, netdata_rwlock_locker *locker)
{
    __netdata_mutex_lock(&rwlock->lockers_mutex);
    ...
    if (!locker->refcount)
    {
        ...
        if (locker->lock == RWLOCK_REQUEST_READ || locker->lock == RWLOCK_REQUEST_TRYREAD)
        {
            rwlock->readers--;
        }
        else if (locker->lock == RWLOCK_REQUEST_WRITE || locker->lock == RWLOCK_REQUEST_TRYWRITE)
        {
            rwlock->writers--;
        }
